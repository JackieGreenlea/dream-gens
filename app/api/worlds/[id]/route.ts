...
import {
  deleteWorldForUser,
  getPlayableByIdOrSample,
  savePlayableSetupForUser,
} from "@/lib/db";
...
export async function GET(
  ...
  const world = await getPlayableByIdOrSample(id, user?.id);
...
export async function PATCH(
  ...
  try {
    ...
    const world = await savePlayableSetupForUser(input, user.id);
...
export async function DELETE(
  ...
  const deleted = await deleteWorldForUser(id, user.id);
